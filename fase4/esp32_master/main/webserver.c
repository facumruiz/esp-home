...
static esp_err_t strip_status(httpd_req_t *req, LedStrip *s) {
    ...
    snprintf(resp, sizeof(resp),
        ...
        s->state ? "true":"false", s->r, s->g, s->b, s->brightness, (int)s->effect);
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
esp_err_t lux_status_handler(httpd_req_t *req) {
    ...
    snprintf(resp, sizeof(resp), "{\"dark\":%s,\"auto\":%s}",
        g_node_status.dark ? "true":"false", global_ctrl->auto_mode ? "true":"false");
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
esp_err_t node_status_handler(httpd_req_t *req) {
    ...
    if (global_ctrl->node_enabled && online) {
        snprintf(resp, sizeof(resp),
            ...
            g_node_status.dark ? "true":"false", g_node_status.motion ? "true":"false",
            global_ctrl->pir_enabled ? "true":"false", global_ctrl->auto_mode ? "true":"false");
    } else {
        snprintf(resp, sizeof(resp),
...
esp_err_t config_status_handler(httpd_req_t *req) {
    ...
    char resp[512];
    snprintf(resp, sizeof(resp),
        "{\"wifi_configured\":%s,\"wifi_connected\":%s,\"wifi_ssid\":\"%s\","
        "\"mqtt_configured\":%s,\"mqtt_connected\":%s,\"mqtt_broker\":\"%s\"}",
        wifi_cfg  ? "true":"false", wifi_conn ? "true":"false", wifi_cfg ? ssid : "",
        mqtt_cfg  ? "true":"false", mqtt_conn ? "true":"false", mqtt_cfg ? broker : "");
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
esp_err_t config_wifi_handler(httpd_req_t *req) {
    ...
    if (!cJSON_IsString(ssid)) { cJSON_Delete(root); httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "SSID requerido"); return ESP_FAIL; }
    config_set_wifi(ssid->valuestring, cJSON_IsString(pass) ? pass->valuestring : "");
    cJSON_Delete(root);
    bool ok = nm_connect_sta();
    char resp[48];
    snprintf(resp, sizeof(resp), "{\"connected\":%s}", ok ? "true":"false");
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
esp_err_t config_mqtt_handler(httpd_req_t *req) {
    char buf[256]={0};
    int received = httpd_req_recv(req, buf, sizeof(buf)-1);
    if (received <= 0) { httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Sin body"); return ESP_FAIL; }
    cJSON *root = cJSON_ParseWithLength(buf, received);
    ...
    if (!cJSON_IsString(broker)) { cJSON_Delete(root); httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Broker requerido"); return ESP_FAIL; }
    config_set_mqtt(broker->valuestring);
    cJSON_Delete(root);
    bool ok = nm_connect_mqtt(global_ctrl);
    char resp[48];
    snprintf(resp, sizeof(resp), "{\"connected\":%s}", ok ? "true":"false");
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp, HTTPD_RESP_USE_STRLEN);
...
esp_err_t config_wifi_clear_handler(httpd_req_t *req) {
    config_clear_wifi();
    nm_disconnect();
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, "{\"ok\":true}", HTTPD_RESP_USE_STRLEN);
...
esp_err_t config_mqtt_clear_handler(httpd_req_t *req) {
    config_clear_mqtt();
    mqtt_manager_stop();
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, "{\"ok\":true}", HTTPD_RESP_USE_STRLEN);
...
esp_err_t root_handler(httpd_req_t *req) {
    ...
    ".color-preview{width:32px;height:32px;border-radius:8px;flex-shrink:0;border:1px solid rgba(255,255,255,.1)}"
    ...
    ".modal{background:var(--card);border-radius:20px;padding:20px;width:100%;max-width:440px;max-height:90vh;overflow-y:auto}"
    ...
    ".inp{width:100%;padding:11px 14px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:var(--text);font-size:.9em;margin-bottom:10px;outline:none}"
    ".inp:focus{border-color:var(--accent)}"
    ".inp:disabled{opacity:.35;cursor:not-allowed}"
    ".cfg-btn{width:100%;padding:12px;border:none;border-radius:12px;font-size:.9em;font-weight:600;cursor:pointer;margin-bottom:8px;transition:all .2s}"
    ".cfg-btn:disabled{opacity:.35;cursor:not-allowed}"
    ".cfg-btn-primary{background:rgba(124,77,255,.3);color:var(--accent);border:1px solid rgba(124,77,255,.4)}"
    ".cfg-btn-danger{background:rgba(255,23,68,.15);color:var(--off);border:1px solid rgba(255,23,68,.3)}"
    ".sbadge{display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:20px;font-size:.72em;font-weight:700;margin-bottom:12px}"
    ...
    "<h1 style='font-size:1.4em;font-weight:300;letter-spacing:2px;color:var(--accent2)'>&#9964; ESP-HOME</h1>"
    "<button onclick='openCfg()' style='background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);cursor:pointer;color:var(--sub);font-size:1.2em;padding:6px 10px;border-radius:10px'>&#9881;</button>"
    "</div>"

    "<div class='two-col'>"
    "<div class='strip-card'>"
    "<div class='card-title'>Tira 1 GPIO18</div>"
    "<div class='state-label'><span class='state-dot off' id='s1dot'></span><span id='s1state'>OFF</span></div>"
    "<div class='row'><button class='btn btn-on' onclick='s1State(\"ON\")'>ON</button><button class='btn btn-off' onclick='s1State(\"OFF\")'>OFF</button></div>"
    "<label>Color</label>"
    "<div class='color-row'><div class='color-preview' id='s1preview' style='background:#ff6400'></div>"
    "<input type='color' id='s1color' value='#ff6400' style='flex:1;height:32px;border:none;border-radius:8px;cursor:pointer;background:none' oninput='s1Color()'></div>"
    ...
    "<div class='row'><button class='btn btn-on' onclick='s2State(\"ON\")'>ON</button><button class='btn btn-off' onclick='s2State(\"OFF\")'>OFF</button></div>"
    "<label>Color</label>"
    "<div class='color-row'><div class='color-preview' id='s2preview' style='background:#ffc864'></div>"
    "<input type='color' id='s2color' value='#ffc864' style='flex:1;height:32px;border:none;border-radius:8px;cursor:pointer;background:none' oninput='s2Color()'></div>"
    ...
    "<div class='card'>"
    "<div class='card-title'>Nodo ESP32 &nbsp;<span class='signal-dot disabled' id='signalDot'></span>&nbsp;<span id='chipNode' class='chip off'>--</span></div>"
    "<div class='sensor-row'>"
    "<div><div style='font-size:.8em;color:var(--sub)'>Luz</div><div style='font-weight:600' id='nodeLux'>--</div></div>"
    "<div style='text-align:right'><div style='font-size:.8em;color:var(--sub)'>PIR</div><div id='nodeMotion' class='chip off'>--</div></div>"
    "</div>"
    "<div class='pir-pill'><div class='pir-dot inactive' id='pirDot'></div><span id='pirLabel' style='font-size:.85em;color:var(--sub)'>Sin movimiento</span></div>"
    "<div class='sw-row'><div class='sw-info'><span class='sw-label'>Sensor PIR</span><span class='sw-sub'>Tira 2 reacciona al movimiento</span></div>"
    "<label class='ios-sw'><input type='checkbox' id='swPir' checked onchange='togglePir()'><span class='ios-track'></span></label></div>"
    "<div class='sw-row'><div class='sw-info'><span class='sw-label'>Modo Auto Luz</span><span class='sw-sub'>Tira 1 responde al sensor de luz</span></div>"
    "<label class='ios-sw'><input type='checkbox' id='swAuto' onchange='toggleAuto()'><span class='ios-track'></span></label></div>"
    "<div class='sw-row'><div class='sw-info'><span class='sw-label'>Nodo ESP-NOW</span><span class='sw-sub'>Habilita la conexion con el nodo</span></div>"
    "<label class='ios-sw'><input type='checkbox' id='swNode' checked onchange='toggleNode()'><span class='ios-track'></span></label></div>"
    "</div>"

    "<div class='card'>"
    "<div class='card-title'>LED Simple GPIO2</div>"
    "<div class='row'><button class='btn btn-on' onclick='fetch(\"/toggle\").then(()=>loadGpio())'>Toggle</button></div>"
    ...
    "<span class='modal-title'>&#9881; Configuracion de red</span>"
    ...
    "<input class='inp' id='inpPass' placeholder='Contrasena WiFi' type='password'>"
    "<button class='cfg-btn cfg-btn-primary' id='btnWifi' onclick='saveWifi()'>Guardar y conectar</button>"
    "<button class='cfg-btn cfg-btn-danger' onclick='clearWifi()'>Borrar configuracion WiFi</button>"
    "<div class='cfg-div'></div>"
    "<div id='mqttSection' style='opacity:.4;pointer-events:none'>"
    ...
    "<button class='cfg-btn cfg-btn-primary' id='btnMqtt' onclick='saveMqtt()'>Guardar y conectar</button>"
    "<button class='cfg-btn cfg-btn-danger' onclick='clearMqtt()'>Borrar configuracion MQTT</button>"
    "</div>"
    "</div></div>"
    ...
    "var on=d.state;document.getElementById('s1dot').className='state-dot '+(on?'on':'off');"
    "document.getElementById('s1state').innerText=on?'ON':'OFF';"
    "var hex=rgbToHex(d.r,d.g,d.b);document.getElementById('s1color').value=hex;"
    "document.getElementById('s1preview').style.background=hex;"
    "document.getElementById('s1bri').value=d.brightness;document.getElementById('s1briVal').innerText=d.brightness;});}"
    "function s1State(s){post('/strip1/set',{state:s}).then(()=>{toast('Tira 1: '+s);loadS1();});}"
    "function s1Color(){var rgb=hexToRgb(document.getElementById('s1color').value);document.getElementById('s1preview').style.background=document.getElementById('s1color').value;post('/strip1/set',{color:rgb});}"
    "function s1Bri(){post('/strip1/set',{brightness:parseInt(document.getElementById('s1bri').value)});}"
    "function s1Fx(ef){document.querySelectorAll('[id^=s1fx]').forEach(b=>b.classList.remove('active'));document.getElementById('s1fx-'+ef).classList.add('active');post('/strip1/set',{effect:ef});}"

    "function loadS2(){fetch('/strip2/status').then(r=>r.json()).then(d=>{"
    "var on=d.state;document.getElementById('s2dot').className='state-dot '+(on?'on':'off');"
    "document.getElementById('s2state').innerText=on?'ON':'OFF';"
    "var hex=rgbToHex(d.r,d.g,d.b);document.getElementById('s2color').value=hex;"
    "document.getElementById('s2preview').style.background=hex;"
    "document.getElementById('s2bri').value=d.brightness;document.getElementById('s2briVal').innerText=d.brightness;});}"
    "function s2State(s){post('/strip2/set',{state:s}).then(()=>{toast('Tira 2: '+s);loadS2();});}"
    "function s2Color(){var rgb=hexToRgb(document.getElementById('s2color').value);document.getElementById('s2preview').style.background=document.getElementById('s2color').value;post('/strip2/set',{color:rgb});}"
    "function s2Bri(){post('/strip2/set',{brightness:parseInt(document.getElementById('s2bri').value)});}"
    "function s2Fx(ef){document.querySelectorAll('[id^=s2fx]').forEach(b=>b.classList.remove('active'));document.getElementById('s2fx-'+ef).classList.add('active');post('/strip2/set',{effect:ef});}"
    ...
    "var chip=document.getElementById('chipNode');chip.innerText=d.enabled?(online?'ON':'OFFLINE'):'OFF';chip.className='chip '+(online?'on':'off');"
    "if(online){"
    "document.getElementById('nodeLux').innerText=d.dark?'OSCURO':'LUZ';"
    "var mc=document.getElementById('nodeMotion');mc.innerText=d.motion?'MOVIMIENTO':'QUIETO';mc.className='chip '+(d.motion?'motion':'off');"
    "document.getElementById('pirDot').className='pir-dot '+(d.motion?'active':'inactive');"
    "document.getElementById('pirLabel').innerText=d.motion?'Movimiento detectado':'Sin movimiento';}"
    ...
    "function openCfg(){document.getElementById('cfgModal').classList.add('open');loadCfgStatus();}"
    "function closeCfg(){document.getElementById('cfgModal').classList.remove('open');}"
    "function loadCfgStatus(){fetch('/config/status').then(r=>r.json()).then(d=>{"
    "var wb=document.getElementById('wifiBadge');"
    "var ms=document.getElementById('mqttSection');"
    "var mb=document.getElementById('mqttBadge');"
    "if(d.wifi_connected){wb.className='sbadge ok';wb.innerText='WiFi: '+d.wifi_ssid;ms.style.opacity='1';ms.style.pointerEvents='auto';}"
    "else if(d.wifi_configured){wb.className='sbadge err';wb.innerText='WiFi: no conectado';ms.style.opacity='.4';ms.style.pointerEvents='none';}"
    "else{wb.className='sbadge err';wb.innerText='WiFi: no configurado';ms.style.opacity='.4';ms.style.pointerEvents='none';}"
    "if(d.mqtt_connected){mb.className='sbadge ok';mb.innerText='MQTT: '+d.mqtt_broker;}"
    "else if(d.mqtt_configured){mb.className='sbadge err';mb.innerText='MQTT: no conectado';}"
    "else{mb.className='sbadge err';mb.innerText='MQTT: no configurado';}"
    "if(d.wifi_ssid)document.getElementById('inpSsid').value=d.wifi_ssid;"
    "if(d.mqtt_broker)document.getElementById('inpBroker').value=d.mqtt_broker;"
    "});}"
    "function saveWifi(){"
    "var ssid=document.getElementById('inpSsid').value.trim();"
    "var pass=document.getElementById('inpPass').value;"
    "if(!ssid){toast('Ingresa el SSID');return;}"
    "var btn=document.getElementById('btnWifi');btn.disabled=true;btn.innerText='Conectando...';"
    "post('/config/wifi',{ssid:ssid,password:pass}).then(r=>r.json()).then(d=>{"
    "btn.disabled=false;btn.innerText='Guardar y conectar';"
    "if(d.connected){toast('WiFi conectado');loadCfgStatus();}else{toast('No se pudo conectar');}"
    "}).catch(()=>{btn.disabled=false;btn.innerText='Guardar y conectar';toast('Error');});}"
    "function saveMqtt(){"
    "var broker=document.getElementById('inpBroker').value.trim();"
    "if(!broker){toast('Ingresa el broker');return;}"
    "var btn=document.getElementById('btnMqtt');btn.disabled=true;btn.innerText='Conectando...';"
    "post('/config/mqtt',{broker:broker}).then(r=>r.json()).then(d=>{"
    "btn.disabled=false;btn.innerText='Guardar y conectar';"
    "if(d.connected){toast('MQTT conectado');loadCfgStatus();}else{toast('No se pudo conectar');}"
    "}).catch(()=>{btn.disabled=false;btn.innerText='Guardar y conectar';toast('Error');});}"
    "function clearWifi(){if(!confirm('Borrar WiFi?'))return;fetch('/config/wifi/clear').then(()=>{toast('WiFi borrado');setTimeout(loadCfgStatus,2000);});}"
    "function clearMqtt(){if(!confirm('Borrar MQTT?'))return;fetch('/config/mqtt/clear').then(()=>{toast('MQTT borrado');loadCfgStatus();});}"

    "loadS1();loadS2();loadNode();loadGpio();"
    "setInterval(loadS1,4000);setInterval(loadS2,4000);setInterval(loadNode,2000);setInterval(loadGpio,6000);"
    "</script></body></html>";
...
void start_webserver(Controller *controller) {
    ...
    httpd_uri_t uris[] = {
        ...
        { .uri="/config/status",     .method=HTTP_GET,  .handler=config_status_handler     },
        { .uri="/config/wifi",       .method=HTTP_POST, .handler=config_wifi_handler       },
        { .uri="/config/mqtt",       .method=HTTP_POST, .handler=config_mqtt_handler       },
        { .uri="/config/wifi/clear", .method=HTTP_GET,  .handler=config_wifi_clear_handler },
        { .uri="/config/mqtt/clear", .method=HTTP_GET,  .handler=config_mqtt_clear_handler },
    };
