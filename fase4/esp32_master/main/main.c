...
void lux_task(void *arg) {
    ...
    while (1) {
        if (ctrl->auto_mode) {
            // Solo usamos el estado que llega del nodo por ESP‑NOW
            bool dark_remote = g_node_status.dark;

            // La tira se enciende únicamente si el nodo reporta oscuridad
            led_strip_ws_set_state(ctrl->strip1, dark_remote);
        }

        // Publicamos igual el estado del sensor local por MQTT (no afecta la lógica)
