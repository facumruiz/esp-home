...
// GET /strip/status → JSON con estado actual
...
// POST /strip/set  → body JSON igual que MQTT
...
esp_err_t root_handler(httpd_req_t *req) {
    const char *html =
        ...
        ".card h2{font-size:1em;color:#a8a8b3;margin-bottom:16px;text-transform:uppercase;letter-spacing:1px}"
        ".row{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-bottom:12px}"
        "button{padding:12px 20px;border:none;border-radius:10px;font-size:0.95em;cursor:pointer;font-weight:bold;transition:opacity .2s}"
        ...
        "input[type=color]{width:100%;height:44px;border:none;border-radius:8px;cursor:pointer;background:none}"
        "#statusBar{text-align:center;padding:8px;border-radius:8px;background:#0f3460;font-size:.85em;margin-top:8px}"
        "</style></head><body>"
        "<h1>&#128161; Control LED</h1>"
        ...
        "<div class='row'>"
        "<button class='btn-on'  onclick='gpioCmd(\"/toggle\")'>Toggle</button>"
        "</div>"
        ...
        "<button class='btn-fx' id='fx-none'    onclick='setEffect(\"none\")'>Estático</button>"
        ...
        // ── GPIO ──
        "function gpioCmd(url){"
        "  fetch(url).then(()=>loadGpioStatus());"
        "}"
        ...
        "  post('/strip/set',{color:rgb}).then(()=>"
        "    setStatus('Color: R'+rgb.r+' G'+rgb.g+' B'+rgb.b));"
        "}"
