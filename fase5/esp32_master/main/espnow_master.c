/*
 * espnow_master.c — versión BLE
 *
 * CAMBIOS respecto a la versión WiFi:
 *   - AÑADIDO: ble_server_notify_node() y ble_server_notify_pir() al recibir
 *     datos del nodo. El teléfono recibe el update automáticamente sin polling.
 *   - AÑADIDO: ble_server_notify_strip2() cuando la tira 2 cambia por PIR.
 *   - Sin cambios en lógica de negocio (PIR timeout, control strip2).
 */

#include "espnow_master.h"
#include "config.h"
#include "led_strip_ws.h"
#include "ble_server.h"           // ← NUEVO: notify cuando llegan datos
#include "esp_now.h"
#include "esp_log.h"
#include "esp_timer.h"
#include <string.h>
...
static void on_data_recv(const esp_now_recv_info_t *info,
    ...
    // Notificar al teléfono inmediatamente (push event-driven)
    ...
    if (g_node_status.motion) {
        ...
        ble_server_notify_strip2();  // ← NUEVO: notificar cambio de strip2
        ...
        if (remaining > 0) {
            ...
            led_strip_ws_apply(s_ctrl->strip2);
            ble_server_notify_strip2();  // ← NUEVO
